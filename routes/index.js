...
router.get("/", (req, res) => {
  ...
  if(req.isAuthenticated()){
    var url = 'http://localhost:5000/user?user='+req.user._id;
  } else {
    var url = 'http://127.0.0.1:5000/users';
  }
  console.log(url)
  request({ url: url, json: true }, function (error, response, body) {
    var stri = body.recommendedProducts;
    var ides = stri.split(",");

    let prods = [];
    async function rec() {
      for (let e = 0; e < ides.length; e++) {
        prods[e] = await Product.findById(ides[e])
      }
      ...
      Product.find().then(products => {
        ...
        if (req.isAuthenticated()) {
          ...
          res.render("routes_UI/index", { productChunks, prods });
        }
      });
